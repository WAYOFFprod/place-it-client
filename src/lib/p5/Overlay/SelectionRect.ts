export default class SelectionRect {
  ...
  updateOverlay(start: Coord, end: Coord, currentScale: number): void {
    ...
    const widthInPixels = Math.abs(Math.round(startPos.x / currentScale) - Math.round(endPos.x / currentScale )) + 1;
    const heightInPixels = Math.abs(Math.round(startPos.y / currentScale) - Math.round(endPos.y / currentScale)) + 1;
  ...
  isInSelection = (x: number, y: number): boolean => {
    return x > this.selection.pos.x && x < this.selection.pos.x + this.selection.size.width
      && y > this.selection.pos.y && y < this.selection.pos.y + this.selection.size.height
  }
