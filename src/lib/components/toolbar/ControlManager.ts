...
export default class ControlManager {
	...
	checkMousePosition() {
		...
		const coords: Coord = {
			x: Math.floor(
				(this.p5.mouseX - this.gridManager.screenOffset.x) / this.gridManager.currentScale
			),
			y: Math.floor(
				(this.p5.mouseY - this.gridManager.screenOffset.y) / this.gridManager.currentScale
			)
		};
	...
	hasNewScreenOffset() {
		...
		if (
			this.previousOffset.x == this.gridManager.screenOffset.x &&
			this.previousOffset.y == this.gridManager.screenOffset.y
		)
			return false;
		return true;
	}
